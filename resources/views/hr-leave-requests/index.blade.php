...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Leave Requests Awaiting HR Approval</h1>
        ...
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('hr-leave-requests.show', $leaveRequest->id) }}" class="text-blue-500 hover:text-blue-700 mr-2">View</a>
                        @if($leaveRequest->status === 'approved_by_manager')
                            <form action="{{ route('hr-leave-requests.approve', $leaveRequest->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-green-500 hover:text-green-700 mr-2">Approve</button>
                            </form>
                            <form action="{{ route('hr-leave-requests.reject', $leaveRequest->id) }}" method="POST" class="inline">
                                @csrf
